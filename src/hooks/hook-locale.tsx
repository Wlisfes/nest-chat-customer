...
import { locale, messages, setI18nLocale, Path, I18nMessager } from '@/i18n'
...
export function useLocale() {
    ...
    //@ts-ignore、重载t方法
    function t(path: I18nContext, props: Refer = {}): string {
        return ctx.t(path, props)
    }

    //@ts-ignore、异步重载t方法
    function at(path: I18nContext, props: Refer = {}): () => string {
        return () => ctx.t(path, props)
    }

    //@ts-ignore、载tm方法
    function tm<T>(path: I18nContext): Array<I18nNode<T>> {
        return ctx.tm(path)
    }

    //@ts-ignore、异步载tm方法
    function atm<T>(path: I18nContext): () => Array<I18nNode<T>> {
        return () => ctx.tm(path)
    }

    //@ts-ignore、列表查找
    function atmNoder<T>(column: Array<T>, value: any, scope: Partial<I18nScopet<T>> = {}): I18nNode<T> {
        const key = scope.valueField ?? 'value'
        const node = column.find(item => (item as I18nNode<T>)[key] == value) ?? scope.defaultValue
        return node as I18nNode<T>
    }

    //@ts-ignore、列表查找转换
    function atmColumnr<T>(column: Array<T>, value: any, scope: Partial<I18nScopet<T>> = {}): string {
